        ...
        public static int FindBestMove(SearchContext context, int depth, int ply, int alpha, int beta, bool allowNullMove, bool friendlyKingInCheck, int extensionsCount)
        {
            ...
            if (entry.Flags != TranspositionTableEntryFlags.Invalid && entry.IsKeyValid(context.BoardState.Hash))
            {
                ...
                if (entry.Depth >= depth)
                {
                    ...
                    switch (entry.Flags)
                    {
                        ...
                        {
                               //EKLEME
                                if (entry.Score < beta /*&& entry.Score < intervalY*/)
                                {
                                    beta = entry.Score;
                                }
                        ...
                        {
                                //EKLEME 
                                //Console.WriteLine($"negamax 138 alpha {alpha} , entry score {entry.Score}");
                                if (entry.Score > alpha /*&& entry.Score < intervalY*/)
                            {
                                alpha = entry.Score;
                            }
            ...
            for (var moveIndex = 0; moveIndex < movesCount; moveIndex++)
            {
                ...
                else
                {
                    ...
                    //EKLEME 
                    //Console.WriteLine($"negamax 376 alpha {alpha} ,  score {score}");
                    
                    if (score > alpha /*&& score < intervalY*/)
                    {
                        if (pvNode)
                        {
                ...
                context.BoardState.UndoMove(moves[moveIndex]);
                //EKLEME
                //Console.WriteLine($"negamax 400 alpha {alpha} , bestScore {bestScore}");
                if (bestScore > intervalY)
                    continue;
                if (bestScore > alpha /*&& bestScore < intervalY*/)
                {
                    alpha = bestScore;
